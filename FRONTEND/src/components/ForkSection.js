...
export function renderForkSection(options = {}) {
    ...
    return `
        ...
        /* The emblems carry their own alpha now — ink on nothing, generated by
           tools/make-step-icons.ps1. They used to be drawn on a cream ground
           that was not #F2E9D2, which multiply DARKENED rather than hid, so
           each icon sat in a visible tile. multiply stays, but it is doing what
           it should now: sinking ink into paper rather than trying to disguise
           a background. Each keeps its own colour, which is why the wax seal is
           still red. */
